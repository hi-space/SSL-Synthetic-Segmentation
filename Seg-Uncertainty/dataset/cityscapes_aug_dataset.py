...
class cityscapesDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.set == 'trainextra':
            ...
            policy = ImageNetPolicy()
            aug_image = policy(image)
            
            image, aug_image, label = np.asarray(image, np.float32), np.asarray(aug_image, np.float32), np.asarray(label, np.uint8)
            ...
            aug_image = aug_image[:, :, ::-1]  # change to BGR
            aug_image -= self.mean
            aug_image = aug_image.transpose((2, 0, 1))
            x1 = random.randint(0, aug_image.shape[1] - self.h)
            y1 = random.randint(0, aug_image.shape[2] - self.w)
            aug_image = aug_image[:, x1:x1+self.h, y1:y1+self.w]

            if self.is_mirror and random.random() < 0.5:
                image = np.flip(image, axis = 2)
                aug_image = np.flip(aug_image, axis = 2)
                label_copy = np.flip(label_copy, axis = 1)
            #print('Time used: {} sec'.format(time.time()-tt))
            return image.copy(), aug_image.copy(), label_copy.copy(), np.array(size), name
