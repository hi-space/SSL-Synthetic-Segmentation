...
class AD_Trainer(nn.Module):
    def __init__(self, args):
        ...
        if args.model == 'DeepLab':
            ...
            self.G_s = Res_Deeplab(num_classes=args.num_classes)
    ...
    def gen_update(self, images, images_t, labels, labels_t, i_iter):
            ...
            # target segmentation model -> source input
            pred1 = self.G(images)
            pred1 = self.interp(pred1)

            # source segmentation model -> target input
            pred2 = self.G_s(images_t)
            pred2 = self.interp(pred2)
            
            if self.class_balance:            
                self.seg_loss1 = self.update_class_criterion(labels)
                self.seg_loss2 = self.update_class_criterion(labels_t)

            if self.only_hard_label > 0:
                labels1 = self.update_label(labels.clone(), pred1)
                loss_seg1 = self.seg_loss1(pred1, labels1)

                labels2 = self.update_label(labels_t.clone(), pred2)
                loss_seg2 = self.seg_loss2(pred2, labels2)
            else:
                loss_seg1 = self.seg_loss1(pred1, labels)
                loss_seg2 = self.seg_loss2(pred2, labels2)
 
            loss = loss_seg1 + loss_seg2

            # target segmentation (trained source input model)
            pred_target1 = self.G(images_t)
            pred_target1 = self.interp_target(pred_target1)

            # source segmentation (trained target input model)
            pred_source = self.G_s(images_t)
            pred_source = self.interp_target(pred_source)

            loss_kl = 0.0
            if self.lambda_kl_target_copy>0:
                n, c, h, w = pred_target1.shape
                with torch.no_grad():
                    mean_pred = self.sm(0.5*pred_source + pred_target1) #+ self.sm(fliplr(0.5*pred_target1_flip + pred_target2_flip)) ) /2
                loss_kl = ( self.kl_loss(self.log_sm(pred_target1) , mean_pred)  + self.kl_loss(self.log_sm(pred_source) , mean_pred))/(n*h*w)
            ...
            val_loss_t = self.seg_loss1(pred_target1, labels_t)
            val_loss_s = self.seg_loss2(pred_source, labels)

            return loss_seg1, loss_seg2, loss_adv_target1, pred_source, pred_target1, val_loss_s, val_loss_t, total_loss
